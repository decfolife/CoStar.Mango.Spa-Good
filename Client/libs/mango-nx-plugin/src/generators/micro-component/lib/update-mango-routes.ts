...
export function updateMangoRoute (
  ...
  `{
    ...
    loadChildren: () =>
    import(
      '../../../mango-crem-features/${options.name}/src/app/components/index/index.module'
      ).then((mod) => mod.IndexModule),
      // data: '{ currentSubApp: MangoSubApps.${names(options.name).constantName}' }
    },
  ...
  const newContents = contents.replace( "// @!micro-component-generator: don't delete this line", newRoute );
  
  if (newContents !== contents) {
    host.write(routingFile, newContents);
  }
  
  console.log("\x1b[32mUPDATED \x1b[0m apps/mango/src/app/app-routing.module.ts with new route");
}