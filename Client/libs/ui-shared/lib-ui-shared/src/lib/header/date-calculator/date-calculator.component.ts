...
export class DateCalculatorComponent implements OnInit, OnDestroy {
  ...
  inputGroup: DxValidationGroupComponent;
  @ViewChild('outputGroup', { static: false })
  outputGroup: DxValidationGroupComponent;

  inputDate: Date;
  includeInputDate = false;
  years = 0;
  months = 0;
  days = 0;
  outputDate: any;
  ...
  public calcDateDifference() {
    ...
    if (validationResult.isValid) {
      if (this.inputDate && this.outputDate) {
        const inputDateTime = this.inputDate.getTime();
        const outputDateTime = this.outputDate.getTime();
