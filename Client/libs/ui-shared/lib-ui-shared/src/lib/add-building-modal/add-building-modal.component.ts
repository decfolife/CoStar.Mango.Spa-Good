  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  @ViewChild('buildingPortfolioId')
  buildingPortfolioIdDropdown: DropdownComponent;
  @ViewChild('buildingTemplateId')
  buildingTemplateIdDropdown: DropdownComponent;
  @ViewChild('buildingHierarchyId')
  buildingHierarchyIdDropdown: DropdownComponent;
  @ViewChild('buildingCountryId') buildingCountryIdDropdown: DropdownComponent;
  @ViewChild('buildingCountryId')
  buildingSubGroupIdIdDropdown: DropdownComponent;
    ...
    if (!this.data.objectTypeName) {
      this.subscriptions.add(
        this.dashboardService
          .getObjectTypeNames([ObjectType.BUILDING])
          .subscribe((result) => {
  ...
  loadDropdownData(): Observable<any> {
    return combineLatest([
      ...
      this.formWizardService.getRenderSelect(
        'United States',
        RequestType.STATES_BY_COUNTRYNAME
      ),
    ]).pipe(
      filter(
      ...
      tap(([countryDropdownItem, portfolioDropdownItem, stateDropDownItem]) => {
  ...
  loadClientHierarchy(): Observable<any> {
    return this.formWizardService
      .getClientPreferenceByField('ClientHierarchyLevel')
      .pipe(
        tap((result) => {
          const mappedValues = result.data.map(
            (clientSetupField) => clientSetupField.clientSetupFieldValue
          );
          this.enableHierachyDropDown = mappedValues?.some(
            (value) =>
              value.toLowerCase().includes('building') ||
              value.toLowerCase().includes('both')
          );
        })
      );
  }
    ...
    return this.formWizardService
      ...
      .pipe(
        tap((result) => {
          ...
          this.enableSubGroupDropDown = mappedValues?.some((value) =>
            value.includes('1')
          );
        })
      );
      ...
      switchMap((companyID) =>
        combineLatest([
          this.formWizardService
            .getRenderSelect(companyID, RequestType.TEMPLATE_ID)
            .pipe(filter((v) => !!v)),
          this.formWizardService
            .getRenderSelect(companyID, RequestType.HIERARCHY_ID)
            .pipe(filter((v) => !!v)),
          this.formWizardService
            .getRenderSelect(companyID, RequestType.SUBGROUP_ID)
            .pipe(filter((v) => !!v)),
        ])
      ),
  ...
  launch() {
    if (this.addBuildingFormGroup.valid) {
      ...
      this.subscriptions.add(
        this.formWizardService.addBuilding(formData).subscribe((result) => {
          if (result.success) {
