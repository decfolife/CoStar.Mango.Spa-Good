...
import {RedirectorLink, AdminFlags, BreadCrumb, Client, ContactRecord, MangoSubApps, UserAuth, UserInfo, V06GlobalSession } from '@mango/data-models/lib-data-models';
...
const appReducer = createReducer(
  ...
  on(AppActions.setupClientSettingsSuccess, (state, { clientInfo }) => ({ ...state, error: null, clientInfo: clientInfo })),
  ...
  on(AppActions.setUserHasSecurityProfiles, (state) => ({ ...state, error: null })),
  on(AppActions.setUserHasSecurityProfilesSuccess, (state, { hasSecurityProfiles }) => ({ ...state, error: null, userHasSecurityProfiles: hasSecurityProfiles })),
  on(AppActions.setEmulatedUser, (state, { initiatedFromV06 }) => ({ ...state, error: null, isEmulateUserInitiatedFromV06: initiatedFromV06 })),
  on(AppActions.setEmulatedUserSuccess, (state, { contactRecord }) => ({ ...state, error: null, contactRecord, isEmulatedUser: true })),
  on(AppActions.stopEmulatingUser, (state, { initiatedFromV06 }) => ({ ...state, error: null, isEmulatedUser: false, isEmulateUserInitiatedFromV06: initiatedFromV06 })),
  ...
  on(AppActions.setAdminFlagsSuccess, (state, {flags}) => ({...state, adminFlags: flags})),
  on(AppActions.loadRedirectorLinksSuccess, (state, { redirectorLinks }) => ({...state, error: null, redirectorLinks: redirectorLinks })),
