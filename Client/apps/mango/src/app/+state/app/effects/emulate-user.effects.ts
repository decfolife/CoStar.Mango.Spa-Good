    ...
    isEmulatingUser$ = createEffect(
        () =>
          this.actions$.pipe(
            ...
            switchMap(_ => this.authService.getEmulatedUser().pipe(
              map(response => {
                if (response.isEmulatedUser) {
                  return AppActions.setEmulatedUser({ contactId: response.contactId, initiatedFromV06: null })
                }
    ...
    setEmulatedUser$ = createEffect(
      () =>
        this.actions$.pipe(
          ...
          switchMap((action: { contactId: number, initiatedFromV06?: boolean }) => combineLatest([of(action.contactId), of(action.initiatedFromV06), this.facade.authenticatedUser$])),
          filter(([_, __, user]) => !!user),
          switchMap(([contactId, initiatedFromV06, user]) => combineLatest([this.userService.getContactRecord(contactId, user.clientKey), of(initiatedFromV06), of(user)])),
          filter(([contactRecord]) => !!contactRecord),
          switchMap(([contactRecord, initiatedFromV06, user]) => this.authService.emulateUser(contactRecord.email, contactRecord.contactID, contactRecord.userRole, user.clientKey).pipe(
            tap(_ => this.facade.setLoading(false)),
            map(_ => {
              return AppActions.setEmulatedUserSuccess({ contactRecord, initiatedFromV06 })
            }),
            catchError(_ => {
              this.facade.setLoading(false)
              this.toaster.show('Could not emulate user.', 'Error', ToastState.ERROR, { position: 'top right' });
              return of(AppActions.noOpAction())
            })
          ))
        )
    ...
    setEmulatedUserSuccess$ = createEffect(
      () =>
        this.actions$.pipe(
          ...
          switchMap((action: { contactRecord: ContactRecord, initiatedFromV06?: boolean }) => combineLatest([of(action.contactRecord), of(action.initiatedFromV06)])),
          filter(([contact, _]) => !!contact),
          tap(([contact, initiatedFromV06]) => {
            if (initiatedFromV06 === null || initiatedFromV06) return

            const searchParams = new URLSearchParams({ cid: contact.contactID.toString(), emu: "true" });
            return this.navigationService.redirectToV06('login.aspx', searchParams)
          })
        ),
      { dispatch: false }
    )
      ...
      () =>
        this.actions$.pipe(
          ...
          switchMap(_ => {
            ...
            return combineLatest([this.facade.isEmulateUserInitiatedFromV06$, this.authService.stopEmulatingUser()])
          }),
