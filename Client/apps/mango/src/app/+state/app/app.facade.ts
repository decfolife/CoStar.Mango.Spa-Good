...
export class MangoAppFacade {
  ...
  constructor(private store: Store) {
    ...
    this.authenticatedUser$ = this.store.pipe(
      select(AppSelectors.authenticatedUser)
    );
    ...
    this.userHasMultipleContactRecords$ = this.store.pipe(
      select(AppSelectors.userHasMultipleContactRecords)
    );
    this.userHasMultipleProfiles$ = this.store.pipe(
      select(AppSelectors.userHasSecurityProfiles)
    );
    ...
    this.isEmulateUserInitiatedFromV06$ = this.store.pipe(
    ...
    this.currentRenderFormDocumentParams$ = this.store.pipe(
      select(AppSelectors.currentRenderFormDocumentParams)
    );
  ...
  oauthAuth(authCode: string, redirectionUri: string, source: string): void {
    this.store.dispatch(
      AppActions.oauthAuth({ authCode, redirectionUri, source })
    );
  }
  ...
  setV06oauthAuth(
    ...
    this.store.dispatch(
      AppActions.setV06Auth({ authCode, redirectionUri, clientKey })
    );
  }
  ...
  updateContactRecord(contactRecord: ContactRecord): void {
    this.store.dispatch(AppActions.updateContactRecord({ contactRecord }));
  }
  ...
  setEmulatedUser(contactId: number, initiatedFromV06: boolean = false): void {
    this.store.dispatch(
      AppActions.setEmulatedUser({ contactId, initiatedFromV06 })
    );
  }

  setEmulatedUserContact(
    contactRecord: ContactRecord,
    initiatedFromV06: boolean = false
  ): void {
    this.store.dispatch(
      AppActions.setEmulatedUserSuccess({ contactRecord, initiatedFromV06 })
    );
  }
  ...
  setCurrentRenderFormDocumentParams(params: string): void {
    this.store.dispatch(
      AppActions.setCurrentRenderFormDocumentParams({ params })
    );
  }
  ...
  setCurrentProjectId(projectId: number): void {
    this.store.dispatch(AppActions.setCurrentProjectId({ projectId }));
  }
}
