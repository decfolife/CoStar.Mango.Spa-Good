...
export class WorkflowAndAlertsComponent implements OnInit, OnDestroy {
  ...
  loading = true;
  env =
    environment.name.toUpperCase().trim() !== 'PROD'
  ...
  ngOnInit() {
    ...
    this.dataService.dashboardDataUpdateKey$.subscribe((data) => {
      if (data) {
        ...
        this.allCardDataLoaded = Object.values(
          this.cardDataLoadCondition
        ).every((item) => item);
      }
    });
    ...
    const dashboardSub = this.dashboardService.DashboardData$.subscribe(
      (result) => {
        if (result.clientSite) {
          ...
          const remEnvironmentSegment = this.env !== '' ? `_${this.env}_` : '_';

          this.exportDataSourceFileName = `CoStar_Data_${
          ...
          if (result.cardsMetaData.length) {
            ...
            this.cardData = result.cardsMetaData.map((card: any) => {
              try {
                ...
                if (
                  card.cardJSONSchema.visible &&
                  !apiEndPoints.includes[card.cardJSONSchema.apiEndPoint]
                ) {
                  this.cardDataLoadCondition[card.cardJSONSchema.apiEndPoint] =
                    false;
              ...
              if (card.cardSource === 'clientdashboard') {
                card.cardJSONSchema.id =
                  'mangoDashboardCardId' + card.mangoDashboardCardId;
                card.sortOrder =
                  this.cardConfigs?.[card.mangoDashboardCardId]?.['sortOrder'];
              ...
              card.cardJSONSchema.pivotDataSource.fields.forEach((field) => {
                if (field.area && field.dataField) {
                  ...
                  if (
                    !columnDictionaryByKey[
                      card.cardJSONSchema.apiEndPoint
                    ].includes(field.dataField)
                  ) {
                    columnDictionaryByKey[card.cardJSONSchema.apiEndPoint].push(
                      field.dataField
                    );
                  }
                }
            ...
            for (const [key, value] of Object.entries(columnDictionaryByKey)) {
              this.dataService.updateInitialColumnData(key, {
                dataSourceKey: key,
                columns: value,
              });
            }
  ...
  public resetDropdownLoadCondition() {
    ...
    Object.keys(this.cardDataLoadCondition).forEach(
      (key) => (this.cardDataLoadCondition[key] = false)
    );
  }
  ...
  public getSecurityLevel(): boolean {
    const userRoleElement = document.getElementById(
      'hasAccountingModuleAddRight'
    );
