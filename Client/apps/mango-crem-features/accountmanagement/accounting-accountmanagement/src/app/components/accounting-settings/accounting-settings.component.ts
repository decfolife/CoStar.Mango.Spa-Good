...
export class AccountingSettingsComponent implements OnInit {
  @ViewChildren(MeasureEventSettingsComponent) private measureEventSettings: QueryList<MeasureEventSettingsComponent>;

  accountingCalendars = [];
  amortizationMethodTypes = [{ Id: 1, Name: 'Periodic' }, { Id: 2, Name: 'Daily' }];
  defaultPaymentTimingTypes = [{ Id: 1, Name: 'Beginning of Period' }, { Id: 2, Name: 'End of Period' }];
  defaultCompoundFrequencyTypes = [{ Id: 1, Name: 'Monthly' }, { Id: 2, Name: 'Daily' }];
  minMonthsOperators = [{ Id: 0, Name: 'Greater than' }, { Id: 1, Name: 'Greater than or equal to' }];
  maxMonthsOperators = [{ Id: 0, Name: 'Less than' }, { Id: 1, Name: 'Less than or equal to' }];
  defaultAnnualRateTypes = [{ Id: 1, Name: 'APR' }, { Id: 2, Name: 'APY' }];
  ...
  functionalCurrencyPeriods = [{ Id: 1, Name: 'Current Period' }, { Id: 2, Name: 'Prior Period' }];
  dropDownValues: any;
  ...
  populateUserRights() {
    this.baseService.getUserRights().subscribe((result) => {
      this.baseService.userRights = Number(result);
  ...
  checkloading() {
    if (this.userRightsLoaded && this.PortfolioClassificationConfigurationLoaded) {
    ...
    this.service.getPortfolioSettings(masterGroupId)
      .subscribe(result => {
        ...
        this.service.portfolioSettings = new PortfolioSettings(
          ...
          settings.functionalCurrencyEnabled, settings.discountRateMatching, settings.directEntryDiscountRateEnabled,
          settings.defaultAnnualRateType, settings.minMonthsOperator, settings.maxMonthsOperator,
          settings.functionalCurrencyRateset, settings.functionalCurrencyPeriod, settings.directEntryFunctionalCurrencyRateEnabled
        );
        ...
        if (this.service.isCalendarInUse === false) {
          if (+settings.leaseRecognitionCalendarID === 1) {
            this.amortizationMethodTypes = this.amortizationMethodTypes.filter(item => item.Name === 'Periodic');
          }
        }

        //show direct entry option, if direct entry is allowed
        if (settings.directEntryFunctionalCurrencyRateEnabled) {
          this.functionalCurrencyRatesets.filter(f => f.Id === 1)[0].visible = true;
        }
      });
      ...
      this.dropDownService.getPortfolioClassificationConfigurationOptions(masterGroupId)
        ...
        this.dropDownValues.journalEntryProfiles.unshift({ profileID: 'Prior Value', profileName: 'Prior Value' });
  ...
  functionalCurrencyRateDirectEntryChanged() {
    if (this.service.portfolioSettings.directEntryFunctionalCurrencyRateEnabled == false) {
      this.functionalCurrencyRatesets.filter(f => f.Id === 1)[0].visible = false;
      if (this.service.portfolioSettings.functionalCurrencyRateset === 'Direct Entry') {
        this.service.portfolioSettings.functionalCurrencyRateset = 'Average Rateset';
      }
    }
    else {
      this.functionalCurrencyRatesets.filter(f => f.Id === 1)[0].visible = true;
  ...
  functionalCurrencyRatesetChanged() {
    ...
    } else if (this.service.portfolioSettings.functionalCurrencyRateset !== 'Direct Entry' &&
               this.service.portfolioSettings.functionalCurrencyPeriod == null ) {
      this.service.portfolioSettings.functionalCurrencyPeriod = this.functionalCurrencyPeriods[0].Name;
    }
  }
  ...
  leaseRecognitionCalendarChanged() {
    if (+this.service.portfolioSettings.leaseRecognitionCalendarID !== 1) {
      ...
      if (this.amortizationMethodTypes.filter(e => e.Name === 'Daily').length <= 0) {
  ...
  saveSettings() {
    ...
    if (!this.shallowEqual(this.service.originalSettings, this.service.portfolioSettings)) {
      this.service.savePortfolioSettings(this.service.portfolioSettings).subscribe(result => {
        if (result > 0) {
          this.service.originalSettings = {...this.service.portfolioSettings}; // update original for continued tracking if necessary
          if (!saveShown) {
            notify({
    ...
    if (configurations.length > 0) {
      this.service.savePortfolioSettingsConfiguration(configurations).subscribe(result => {
        if (result > 0) {
          this.measureEventSettings.forEach(f => {
