...
import { DistributionListKeys, DistributionListMember } from '@mango/data-models/lib-data-models';
...
export class DistributionListsMembersComponent {
	...
	@Output() selectedDistributionListAndMembersEvent: EventEmitter<any> = new EventEmitter();
	@Output() getLatestDistributionListDataEvent: EventEmitter<any> = new EventEmitter();
	...
	@ViewChild("DistributionListsMembersGrid") distributionListsMembersGrid: DxDataGridComponent;
	@ViewChild("distributionListMemberActionsMenuTrigger") actionsMenuTrigger: MatMenuTrigger;
  ...
  constructor(
	...
	removeDistributionListsMember(member: DistributionListMember) {
		...
		if (this.memberCount == 1) {
			let message = 'Distribution list member removal can not be done. At least one distribution list member must be assigned to the distribution list.';
			this.subs.push(this.dialogService.alert('Distribution List Member Removal', message, 'OK').subscribe());
		} else {
			let confirmText = `Do you want to remove the member "${member.name}" ?`;
			this.subs.push(this.dialogService.confirm('Remove Distribution List Member', confirmText, 'Confirm', 'Cancel').pipe(
				filter(confirmed => !!confirmed),
				switchMap(_ => this.distributionListsService.deleteMembers(this.memberIds)),
				switchMap(res => {
					if (res.success) {
						this.memberIds = [];
						this.getLatestDistributionListDataEvent.emit();
					}
					return res.success ? of(this.distributionListsService.successNotify("Selected member(s) successfully removed."))
						: this.dialogService.alert('Removal unsuccessful!', 'Distribution list member could not be deleted. Please review and try again later.', 'OK');
				})
			).subscribe());
		}
	}
	...
	onSelectionChanged(e: any) {
		if (e.currentSelectedRowKeys.length) {
			...
			if (this.selectedListAndMembersData.groupID) {
				this.selectedListAndMembersData.memberIds = this.selectedListAndMembersData.memberIds.concat(e.currentSelectedRowKeys);
			} else {
		...
		if (e.currentDeselectedRowKeys.length) {
			...
			this.selectedListAndMembersData.memberIds =
				this.selectedListAndMembersData.memberIds.filter(item => !(e.currentDeselectedRowKeys).includes(item));
		}
		this.selectedDistributionListAndMembersEvent.emit(this.selectedListAndMembersData);
	}
  ...
  onKeyDown(event) {
    ...
    if (event.event.key === 'Enter' || event.event.key === ' ') {
      ...
      if (focusedColumn !== undefined && focusedIndex !== -1) {
        ...
        if (datafield === 'role' || datafield === 'level') {
          const srcElement = gridInstance.getCellElement(focusedIndex, datafield);
          if (srcElement) {
	...
	gridOnCellPrepared(e) {
		if (e.column.command == 'select' && (this.rights.toLocaleLowerCase().trim() == "view")) {
