...
export class GroupUserHistoryComponent implements OnInit {
  ...
  public userPreferencesResolved: ResolvedData =
  ...
  public apply(): void {
    ...
    if (this.selectedUsers?.length) {
      this.tabs.push({ title: 'User History', template: 'userHistory' });
    }

    if (this.selectedGroups?.length) {
      this.tabs.push({ title: 'Group History', template: 'groupHistory' });
    }
    this.isApplied = true;
  ...
  public exportGrids() {
    ...
    const transformedPeriodFrom = this.datepipe.transform(
      this.periodFrom,
      this.dateFormat
    );
    const transformedPeriodTo = this.datepipe.transform(
      this.periodTo,
      this.dateFormat
    );
        ...
        .then(() => {
          ...
          workbook.xlsx.writeBuffer().then((buffer) => {
            saveAs(
              ...
              'Group and User History - ' + date + '.xlsx'
            );
          });
