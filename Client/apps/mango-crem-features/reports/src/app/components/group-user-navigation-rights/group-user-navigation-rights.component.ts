...
export class GroupUserNavigationRightsComponent implements OnInit {
  ...
  public selectedUserId: any = +this.route.snapshot.paramMap.get('userId') || undefined;
  public selectedGroupId: any = +this.route.snapshot.paramMap.get('groupId') || undefined;
  public selectedModuleId: any = +this.route.snapshot.paramMap.get('moduleId') || undefined;
  ...
  ngOnInit(): void {
    ...
    this.initialUser = this.selectedUserId ? this.userList.find(x => x.id === this.selectedUserId): null;
    this.initialGroup = this.selectedGroupId ? this.groupList.find(x => x.id === this.selectedGroupId): null;
    this.initialModule = this.selectedModuleId ? this.moduleList.find(x => x.id === this.selectedModuleId): null;
    if (this.initialUser) {
    ...
    if (this.userListResolved.error || this.groupListResolved.error || this.moduleListResolved.error) {
  ...
  public apply() {
    ...
    this.getNavigationRightsData(selectedUserIds, selectedGroupIds, selectedModuleIds);
  }

  private getNavigationRightsData(selectedUserIds: any, selectedGroupIds: any, selectedModuleIds: any): void {
    this.service.getNavigationRightData(selectedUserIds, selectedGroupIds, selectedModuleIds)
      .subscribe(result => {
        ...
        if (result.success) {
          ...
          this.dataGridList.sort((a, b) => {
            if(a.moduleDisplayName?.toLowerCase() < b.moduleDisplayName?.toLowerCase()) { return -1; }
            if(a.moduleDisplayName?.toLowerCase() > b.moduleDisplayName?.toLowerCase()) { return 1; }
            return 0;
          });
  ...
  public onCellPrepared(e) {
    if(e.rowType === "data") {
      if (e.data[e.column.dataField] && e.data[e.column.dataField].toString().toLowerCase().startsWith("view -")) {
        // e.cellElement.style.backgroundColor = "#dff0d8"
        e.cellElement.classList.add("bg-success-lightest");
      }

      if (e.data[e.column.dataField] && e.data[e.column.dataField].toString().toLowerCase().startsWith("edit -")) {
        // e.cellElement.style.backgroundColor = "#fcf8e3"
        e.cellElement.classList.add("bg-warning-lightest");
      }

      if (e.data[e.column.dataField] && e.data[e.column.dataField].toString().toLowerCase().startsWith("delete -")) {
        // e.cellElement.style.backgroundColor = "#f2dede"
        e.cellElement.classList.add("bg-danger-lightest");
      }

      if (e.data[e.column.dataField] && e.data[e.column.dataField].toString().toLowerCase().startsWith("block -")) {
        // e.cellElement.style.backgroundColor = "#d9edf7"
        e.cellElement.classList.add("bg-info-lightest");
      }

      if (e.data[e.column.dataField] && e.data[e.column.dataField].toString().toLowerCase().includes("inherited")) {
  ...
  public exportDynamicGrids(e) {
    ...
    const setBackground = (gridCell, excelCell) => {
        ...
        excelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'd2d2d2' }, bgColor: { argb: 'd2d2d2' }};
			...
			if (gridCell.rowType === 'data') {
				...
				if (gridCell?.data && gridCell?.data[gridCell?.column.dataField]?.toString().toLowerCase().startsWith("view -")) {
					excelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'dff0d8' }, bgColor: { argb: 'dff0d8' }};
        }

        if (gridCell?.data && gridCell?.data[gridCell?.column.dataField]?.toString().toLowerCase().startsWith("block -")) {
					excelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'd9edf7' }, bgColor: { argb: 'd9edf7' }};
        }

        if (gridCell?.data && gridCell?.data[gridCell?.column.dataField]?.toString().toLowerCase().startsWith("delete -")) {
					excelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'f2dede' }, bgColor: { argb: 'f2dede' }};
        }

        if (gridCell?.data && gridCell?.data[gridCell?.column.dataField]?.toString().toLowerCase().startsWith("edit -")) {
					excelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'fcf8e3' }, bgColor: { argb: 'fcf8e3' }};
        }

        if (gridCell?.data && gridCell?.data[gridCell?.column.dataField]?.toString().toLowerCase().includes("inherited")) {
          if (excelCell.font) {
    ...
    const exportDataGridCallback = (iteration, row) => {
      if (iteration < this.dataGridList.length) {
        navigationRightsSheet.getRow(row).getCell(2).value = this.dataGridList[iteration]?.moduleDisplayName;
        ...
        filterSheet = this.buildFilterSheet(filterSheet, 2, 'Users', this.selectedUsers,  'name');
        filterSheet = this.buildFilterSheet(filterSheet, 3, 'Groups', this.selectedGroups,  'name');
        filterSheet = this.buildFilterSheet(filterSheet, 4, 'Modules', this.selectedModules,  'name');
        ...
        exportDataGrid({
          ...
          workbook.xlsx.writeBuffer().then((buffer) => {
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Navigation Rights Report - ' + date +'.xlsx');
          });
        });
      }
  ...
  private getErrorMessageList(): void {
    if (this.userListResolved.error) {
      this.errorMessageList.push('UserListError: ' + this.userListResolved.error);
    }
    if (this.groupListResolved.error) {
      this.errorMessageList.push('GroupListError: ' + this.groupListResolved.error);
    }
    if (this.moduleListResolved.error) {
      this.errorMessageList.push('ModuleListError: ' + this.moduleListResolved.error);
    }
  }
