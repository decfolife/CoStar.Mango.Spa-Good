...
export class ExchangeRateSetsComponent implements OnInit {
  ...
  public selectedMasterGroupID: number =
    +this.route.snapshot.paramMap.get('portfolioId') || undefined;
  ...
  ngOnInit(): void {
    ...
    if (this.selectedMasterGroupID) {
      this.selectedPortfolio = this.portfolioList.find(
        (x) => x.masterGroupID === this.selectedMasterGroupID
      );
    }
  ...
  public getPeriodList(): void {
    this.exchangeRateSetsService
      .getPeriodList(this.selectedPortfolio.masterGroupID)
      .subscribe((result) => {
  ...
  public getExchangeRateSetData(): void {
    this.exchangeRateSetsService
      .getExchangeRateSetData(
        this.selectedPortfolio.masterGroupID,
        this.selectedPeriods
      )
      .subscribe((result) => {
        this.exchangeRateData = (result.data || []).map((d, index) => ({
          ...d,
          gridIndex: index,
        }));
  ...
  public onPortfolioChange(e: any[]): void {
    const selectedPortfolio: Portfolio = e?.[0] && {
      masterGroupID: e?.[0]?.valueKey,
  ...
  public exportGrids(): void {
    ...
    rateSetSheet.getRow(2).getCell(2).font = {
      bold: true,
      size: 16,
      underline: 'double',
    };
    ...
    rateSetSheet.getRow(2).getCell(5).value =
      this.selectedPortfolio.companyName;
  ...
  private builderPortfolioDropdown(): void {
    ...
    if (this.selectedPortfolio) {
      this.onPortfolioChange([
        {
          valueKey: this.selectedPortfolio.masterGroupID,
          companyName: this.selectedPortfolio.companyName,
        },
      ]);
    }
  }
