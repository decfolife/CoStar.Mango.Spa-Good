        ...
        observableList.subscribe((data: any) => {
            ...
            if (data.reports?.data) {
                ...
                if (data.criteriaSets?.data) {
                    this.allowSegments = data.criteriaSets.data.find(x => x.criteriaSetID === this.criteriaSetId)
                }
                if (this.allowSegments) {
                    this.reportsService.getSegments(this.criteriaSetId, false).subscribe((result) => {
            ...
            this.loadSegmentValues({
                ...
                values: {
                    segmentID: {
                        caption: "Segment",
                        dataField: "segmentID",
                        value: [{segmentID: this.data.redirectData?.newSegmentID, portfolioID: this.data.redirectData?.segmentValues.PortfolioID}]
                    }
                }
            })
    ...
    private loadSegmentValues(config) {
        if (config.values["segmentID"]?.value?.length) {
            ...
            setTimeout(() => {
                ...
                this.defaultValues['PortfolioID'] = config.values["segmentID"]?.value[0].portfolioID;
            ...
            saveObject.forEach(x => {
                if (config[x.FieldName]) {
                    if(config[x.FieldName].fieldType === "toFromDate") {
                        newSegmentConfig[config[x.FieldName]?.data.criteriaID1] = config[x.FieldName].value1
                        newSegmentConfig[config[x.FieldName]?.data.criteriaID2] = config[x.FieldName].value2
                    } else {
