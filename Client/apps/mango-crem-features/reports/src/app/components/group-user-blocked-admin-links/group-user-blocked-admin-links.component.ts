...
export class GroupUserBlockedAdminLinksComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.service.getBlockedAdminLinkData().subscribe((result) => {
  ...
  public exportGrids() {
    ...
    const blockedAdminLinksSheet = workbook.addWorksheet(
      'Group and User Blocked Admin Links'
    );
    const filterSheet = workbook.addWorksheet('Filters');

    blockedAdminLinksSheet.getRow(2).getCell(2).value =
      'Group and User Blocked Admin Links';
    blockedAdminLinksSheet.getRow(2).getCell(2).font = {
      bold: true,
      size: 16,
      underline: 'double',
    };
      ...
      .then(() => {
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
          saveAs(
            ...
            'Group and User Blocked Admin Links - ' + date + '.xlsx'
          );
        });
      });
  }
      ...
      if (column.filterType !== 'exclude') {
        ...
        for (var j = 0; j < filterValues.length; j++) {
          ...
          filterExpression.push(
            column.calculateFilterExpression(filterValues[j], '<>')
          );
        }
        ...
        this[grid].instance
          ...
          .load({
            ...
            group: { selector: column.dataField, isExpanded: false },
          })
          .done(function (data) {
