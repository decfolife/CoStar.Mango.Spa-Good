...
export class GroupListResolver implements Resolve<ResolvedData> {

    constructor(private service: SharedService) { }

    resolve(route: ActivatedRouteSnapshot,
        state: RouterStateSnapshot): Observable<ResolvedData> {
        return this.service.getReportGroupList()
            .pipe(
                map(result => ({ data: result.data })),
                catchError(error => {
                    const message = `GroupListResolver Retrieval error: ${error}`;
