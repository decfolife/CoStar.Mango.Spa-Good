...
import { faFolderOpen, faExclamationCircle, faArrowUp, faEllipsisH } from '@fortawesome/free-solid-svg-icons';
...
export class PortfolioMaintenanceComponent implements OnInit {
  ...
  constructor(
    ...
    private dialog: MatDialog
      ...
      const portfolioSubscription = this.service.getPortfolioHierarchyList()
        .subscribe(result => {
          this.portfolioMaintenanceData = result.data.portfolioHierarchies.filter((data) => {
  ...
  private setCompanyNameDisplay() {
    this.portfolioMaintenanceData.forEach((data) => {
      data.companyNameDisplay = (data.parentGroup ? data.companyLevelLabel + ": " : "") + data.companyID + " - " + data.companyName + (data.isActive ? "" : " (Archived)")
    })
  }
  ...
  public onCellClicked(e): void {
    if (!(e.event.target.parentElement.classList.contains("dx-treelist-expanded") 
        ...
        || e.event.target.parentElement.classList.contains("dx-treelist-icon-container")
        || e.event.target.parentElement.classList.contains("dx-texteditor-input-container")
        || e.event.target.parentElement.classList.contains("dx-treelist-form-buttons-container")
  ...
  exportTreeList() {
    ...
    const portfolioMaintenanceSheet = workbook.addWorksheet('Portfolio', { views: [{ showGridLines: false }] });
    ...
    portfolioMaintenanceSheet.getRow(2).getCell(2).font = { bold: true, size: 16, underline: 'double' };

    portfolioMaintenanceSheet.mergeCells("B4:G4")
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Portfolio Name", 4, 2, "B4", 4);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Info", 4, 8, "H4", 6);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Master Group ID", 4, 9, "I4", 15);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Company ID", 4, 10, "J4", 12);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Company Name", 4, 11, "K4", 15);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Company Hierarchy Level1", 4, 12, "L4", 24);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Company Hierarchy Level2", 4, 13, "M4", 24);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Company Hierarchy Level3", 4, 14, "N4", 24);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Company Hierarchy Level4", 4, 15, "O4", 24);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Company Hierarchy Level5", 4, 16, "P4", 24);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Company Level", 4, 17, "Q4", 13);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Company Level Label", 4, 18, "R4", 24);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Grouping Levels", 4, 19, "S4", 15);
    this.setColumnHeaderStyle(portfolioMaintenanceSheet, "Parent Group", 4, 20, "T4", 12);

    const objectTreeList = this.buildObjectTreeList(this.portfolioMaintenanceData);
  ...
  private setColumnHeaderStyle(portfolioMaintenanceSheet, value, row, column, cell, rowWidth) {
    portfolioMaintenanceSheet.getColumn(column).width = rowWidth;
    portfolioMaintenanceSheet.getRow(row).getCell(column).value = value;
    portfolioMaintenanceSheet.getRow(row).getCell(column).font = { color: { argb: '00558E' } }
    portfolioMaintenanceSheet.getRow(row).getCell(column).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'd2d2d2' }, bgColor: { argb: 'd2d2d2' } };
  ...
  private addExportData(portfolioMaintenanceSheet, data) {
    ...
    data.forEach((item) => {
      column = 2;
      if (item.data.companyID.toString().length + item.data.companyNameDisplay.length - 17 > maxWidth) {
        maxWidth = item.data.companyID.toString().length + item.data.companyNameDisplay.length - 17;
      }
      ...
      if (item.childItems.length) {
        item.childItems.forEach((childItem1) => {
          row++;
          column = 3;
          if (childItem1.data.companyID.toString().length + childItem1.data.companyNameDisplay.length - 13 > maxWidth) {
            maxWidth = childItem1.data.companyID.toString().length + childItem1.data.companyNameDisplay.length - 13;
          }
          this.addCellData(portfolioMaintenanceSheet, "C", row, column, childItem1)
          if (childItem1.childItems.length) {
            childItem1.childItems.forEach((childItem2) => {
              row++;
              column = 4;
              if (childItem2.data.companyID.toString().length + childItem2.data.companyNameDisplay.length - 9 > maxWidth) {
                maxWidth = childItem2.data.companyID.toString().length + childItem2.data.companyNameDisplay.length - 9;
              }
              this.addCellData(portfolioMaintenanceSheet, "D", row, column, childItem2)

              if (childItem2.childItems.length) {
                childItem2.childItems.forEach((childItem3) => {
                  row++;
                  column = 5;
                  if (childItem3.data.companyID.toString().length + childItem3.data.companyNameDisplay.length - 5 > maxWidth) {
                    maxWidth = childItem3.data.companyID.toString().length + childItem3.data.companyNameDisplay.length - 5;
                  }
                  this.addCellData(portfolioMaintenanceSheet, "E", row, column, childItem3)
                  if (childItem3.childItems.length) {
                    childItem3.childItems.forEach((childItem4) => {
                      row++;
                      column = 6;
                      if (childItem4.data.companyID.toString().length + childItem4.data.companyNameDisplay.length - 1 > maxWidth) {
                        maxWidth = childItem4.data.companyID.toString().length + childItem4.data.companyNameDisplay.length - 1;
                      }
                      this.addCellData(portfolioMaintenanceSheet, "F", row, column, childItem4)
                      if (childItem4.childItems.length) {
                        childItem4.childItems.forEach((childItem5) => {
                          row++;
                          column = 7;
                          if (childItem5.data.companyID.toString().length + childItem5.data.companyNameDisplay.length + 3 > maxWidth) {
                            maxWidth = childItem5.data.companyID.toString().length + childItem5.data.companyNameDisplay.length + 3;
                          }
                          this.addCellData(portfolioMaintenanceSheet, "G", row, column, childItem5)
                        });
                      }
                    });
                  }
  ...
  private addCellData(portfolioMaintenanceSheet, nameCell, row, column, item) {
    portfolioMaintenanceSheet.getRow(row).getCell(column).value = item.data.companyNameDisplay;
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "I", row, 9, "masterGroupID", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "J", row, 10, "companyID", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "K", row, 11, "companyName", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "L", row, 12, "companyHierarchyLevel1", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "M", row, 13, "companyHierarchyLevel2", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "N", row, 14, "companyHierarchyLevel3", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "O", row, 15, "companyHierarchyLevel4", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "P", row, 16, "companyHierarchyLevel5", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "Q", row, 17, "companyLevel", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "R", row, 18, "companyLevelLabel", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "S", row, 19, "groupingLevels", item.data);
    this.setCellDataAndStyle(portfolioMaintenanceSheet, "T", row, 20, "parentGroup", item.data);
  ...
  public setCellDataAndStyle(portfolioMaintenanceSheet, nameCell, row, column, columnName, data) {
    portfolioMaintenanceSheet.getRow(row).getCell(column).value = data[columnName];
    portfolioMaintenanceSheet.getCell(nameCell + row).border = {
      top: { style: 'thin' },
      left: { style: 'thin' },
      bottom: { style: 'thin' },
    ...
    if (data[columnName]?.length > columnWidth) {
      portfolioMaintenanceSheet.getColumn(column).width = data[columnName].length;
    }
  }
  ...
  public inlineAddOrEdit(rowIndex, data, action) {
    switch (action) {
      case 'addSubPortfolio': {
        ...
        this.service.getCompanyHierarchyLabels(data.masterGroupID).subscribe((result) => {
          const hierarchyLabel = result.data.find((label) => {
            return label.companyHierarchyLabelOrder === (data.companyLevel + 1);
          })
          this.companyNameInputText = hierarchyLabel?.companyHierarchyLabelDesc;
  ...
  public setInlineDropdownDisplayCondition(data) {
    if (data.companyLevel === 0) {
      ...
      this.inlineEditDropdownData = this.portfolioMaintenanceData.filter((item) => {
  ...
  public onRowUpdated(event, action) {
    ...
    this.service.saveSubHierarchy(request)
      .subscribe(() => {
        this.service.getPortfolioHierarchyList()
          .subscribe(result => {
            this.portfolioMaintenanceData = result.data.portfolioHierarchies.filter((data) => {
              return data.companyID !== -1;
            });
  ...
  public redirectToAction(data, action) {
    ...
    switch (action) {
      ...
      case 'editPortfolio': {
        route = `${this.basepageUrl}editPortfolio/${data.masterGroupID}/${data.companyID + basewebPage}`
        break;
      }
      case 'showData': {
        route = `${this.basepageUrl}${this.isPortfolio(data) ? 'portfolioAssociatedObjects' : 'noPortfolioAssociatedObjects'}/${data.masterGroupID}/${data.companyID + basewebPage}`
        break;
      }
      case 'changeCurrency': {
  ...
  public viewContacts(data) {
    this.service.getGroupContacts(data.masterGroupID)
    .subscribe((result) => {
    ...
    let errorObject: any = {
      ...
      if (isPortfolio) {
        ...
        if (!data.isActive) {
          ...
          this.openDeleteModal(data, true, false, modalConfiguration, errorObject, "600px")
        } else {
            /*possible use cases
                1.	Happy path – 200 OK
              ...
              */
            this.service.archivePortfolio(data.masterGroupID)
            .subscribe(result => {
            
             if(!result.success){
              //clientErrorMessage has the status code of the error
              switch (result.clientErrorMessage) {
                // use case 4 - bad request
                case 400:
                case "400": {
                  errorObject.hasRightsLinked = true;
                  errorObject.rights = result.data;
            ...
            this.openDeleteModal(data, true, false, modalConfiguration, errorObject, width)
          });
        }
      
      } else {
        this.service.hasData(data.parentGroup, data.companyID, isPortfolio)
        .subscribe(result => {
          ...
          this.openDeleteModal(data, false, hasData, modalConfiguration, errorObject, width)
        });
    }
  }
  ...
  public openDeleteModal(data, isPortfolio, hasData, modalConfiguration, errorObject, width) {
    const modalObject = {
      isPortfolio,
      hasData,
      data,
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        if (!result.isPortfolio) {
          ...
          this.service.deleteCompany(data.parentGroup, data.companyID, isPortfolio)
            .subscribe(() => {
              this.deleting = false;
            });
            const index = this.portfolioMaintenanceData.findIndex((item) => {
              return item.companyID.toString() === result.data.companyID?.toString() && !(item.companyLevel === 0)
            });
            this.portfolioMaintenanceData.splice(index, 1);
          } else {
            //path for deleting portfolio
            const index = this.portfolioMaintenanceData.findIndex((item) => {
              return item.companyID.toString() === result.data.companyID?.toString();
            });
            if (result.data.isActive) {
              //service call involving archiving portfolio go here
              this.service.archivePortfolio(data.masterGroupID, true)
              .subscribe(() => {
                // Align the UI for the archived row without refreshing the grid
                this.portfolioMaintenanceData[index].enableArchivePortfolio = false;
                this.portfolioMaintenanceData[index].enableDeletePortfolio = true;
              ...
              this.service.deletePortfolio(data.masterGroupID, result.deleteAllContactsAndGroups, data.companyName)
                .subscribe(() => {});
              this.portfolioMaintenanceData.splice(index, 1);
            }
          }
