...
export class PortfolioMaintenanceService extends EndpointService {
  portfolioMaintenanceUrl: string = UtilitiesService.getBaseApiUrl(Api.portfolioMaintenance)
  ...
  deleteCompany(portfolioId: number, companyId: number, isPortfolio: boolean): Observable<any> {
      const url = `${this.portfolioMaintenanceUrl}PortfolioMaintenance/Delete/${companyId.toString()}`;
      return this.callHttpPost(url, 'delete', {})
  }

  hasData(portfolioId: number, companyId: number, isPortfolio: boolean): Observable<any> {
    const url = `${this.portfolioMaintenanceUrl}PortfolioMaintenance/HasAssociatedData/${portfolioId.toString()}/${companyId.toString()}/${isPortfolio}`;
  ...
  archivePortfolio(portfolioId: number, canArchive: boolean = false): Observable<any> {
    const url = `${this.portfolioMaintenanceUrl}PortfolioMaintenance/Archive/${portfolioId.toString()}/${canArchive}`;
