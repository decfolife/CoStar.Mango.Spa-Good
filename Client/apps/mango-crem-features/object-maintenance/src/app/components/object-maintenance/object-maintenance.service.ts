...
export class ObjectMaintenanceService extends EndpointService {
  objectMaintenanceUrl: string = UtilitiesService.getBaseApiUrl(
    Api.objectMaintenance
  );
  ...
  onStatusFilterChangeChange(objectTypeTypeId: number, statusFilterId: number) {
    ...
    this.getFilteredPageData(objectTypeTypeId, statusFilterId).subscribe(
      (result) => {
        this._pageData.next(result);
        this._loading.next(false);
      }
  ...
  private getFilteredPageData(
    objectTypeId: number,
    statusFilterVal: number
  ): any {
