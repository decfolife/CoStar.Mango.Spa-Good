...
export class ObjectMaintenanceComponent implements OnInit, OnDestroy {
  ...
  public currentObjectTypeId: number = +this.route.snapshot.paramMap.get('OTID') || undefined;
  ...
  constructor(
    ...
    private service: ObjectMaintenanceService,) {
      const loadingSub = this.service.isLoading$.subscribe(isLoadingVal => {this.isLoading = isLoadingVal});
      const objectTypeSub = this.service.objectTypeDropdownData$.subscribe(objectTypes => {
        if (objectTypes) {
  ...
  public onRowClicked(item): void {
    ...
    if (item.data.canEdit) {
      const route = "/v06/Admin/SecurityMaintenance/SecurityMaintenance.aspx#/assignRights/" + this.currentObjectTypeId + "/" + item.data.objectId +"/ObjectMaintenance.aspx"
      window.location.href = route;
    }
  }

  public onCellPrepared(e) {  
    if (e.rowType == "data" && (e.column.dataField === "objectId" || e.column.dataField == "objectName" || e.column.dataField === "portfolio" || e.column.dataField === "country" || e.column.dataField === "template" || e.column.dataField === "rights" || e.column.dataField === "actions" )){  
        if (!e.data.canEdit) {
  ...
  public onFilterChange(status: number) {
    ...
    this.service.onStatusFilterChangeChange(this.currentObjectTypeId, status?.[0]?.intValue);
  }
  ...
  public exportGrids(): void {
    ...
    const objectStatus = "Object Status: " + this.statusDictionary[this.currentFilterType];
  ...
  private buildGridColumns(showPortfolioCountry: boolean = false, showTemplatecolumn: boolean = false) {
    this.columns = [
      {
        dataField: "objectId",
        alignment: "left",
