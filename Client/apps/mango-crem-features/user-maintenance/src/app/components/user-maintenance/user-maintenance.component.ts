...
export class UserMaintenanceComponent implements OnInit {
  ...
  public getUserList(filter) {
    this.service.getUserList(filter).subscribe((result) => {
  ...
  public getUserFormId() {
    this.service.getUserFormId().subscribe((result) => {
  ...
  public deleteUser(data) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.deleting = true;
        this.service.deleteUser(result.contactID).subscribe((result) => {
          this.deleting = false;
        });
        const index = this.userMaintenanceData.findIndex((item) => {
          return (
            item.contactID.toString() === result?.contactID?.toString() &&
            !item.isContact
          );
        });
