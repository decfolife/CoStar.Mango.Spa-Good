...
export class BookmarksService extends EndpointService {
    ...
    return combineLatest([
      ...
      map(
        ([
          ...
          let reports = bookmarksData.data.reportsData.map(
            (rd) =>
              new Bookmark(
                rd.reportID,
                null,
          ...
          this.bookmarks = bookmarksData.data.bookmarksData.map(
            (bd) =>
              new Bookmark(
                bd.bookmarkID,
                bd.objectID,
                ...
                false,
                bd.bookmarkText,
                this.findUrl(bd.objectID, bd.objectTypeID, bd.objectTypeTypeID)
              )
          );
          ...
          if (bookmarksData.data) {
            filteredBookmarkGroupsObjectTypeIdList.forEach((id) => {
              let objectTypeName = objectTypeNameData.data.find(
                (otnd) => otnd.objectTypeId === id
              ).objectTypeName;
              let filteredBookmarks =
                id === 7 ? reports : this.filterBookmarks(objectTypeName);
              let favGroup = new BookmarkGroup(
                id,
                objectTypeName,
                filteredBookmarks
              );
  ...
  private filterBookmarks(object_type): Bookmark[] {
    return this.bookmarks.filter((item) => item.objectType == object_type);
  }
