...
export class RemindersListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.currentUserRole$ = this.facade.contactRecord$.pipe(
      filter((contact) => !!contact),
      map((contact) => contact.userRole)
    );
  ...
  public setReminderColumns() {
    this.columns = [
          ...
          const customizedData =
            options.data.TicklerTypeID === 1 && hasUserDefinedDate
              ? options.data.Ticklername +
                ' ' +
                '(' +
                options.data.UserDefinedDate.split('T')[0] +
                ')'
              : options.data.Ticklername;
      ...
      },
      {
        ...
        cellTemplate: (container, options) => {
          container.innerText =
            options.data.ContactFirstName + ' ' + options.data.ContactLastName;
        },
      },
      {
  ...
  public performDeleteReminder(reminderObj: any) {
    this.subscriptions.add(
      this.reminderService
        .deleteReminder(
          reminderObj.TicklerID,
          reminderObj.ObjectID,
          reminderObj.ObjectTypeID
        )
        .subscribe(
          (res: any) => {
            if (res.success) {
              this.gridData = this.gridData.filter(
                (reminder) => reminder.TicklerID !== reminderObj.TicklerID
              );
            } else {
