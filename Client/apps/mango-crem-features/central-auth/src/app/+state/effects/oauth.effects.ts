...
export class OAuthEffects {
  ...
  initAuthorization$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        switchMap(
          ([payload, isClientSpecificLogin]: [
            MultiClientLoginHttpRequest,
            boolean
          ]) =>
            this.authService.loginToClientSite(payload).pipe(
              catchError((e) => {
  ...
  logoutWhenIsClientSpecificLoginAndloginToClientSiteFailed$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(
          AppActions.LOG_OUT_WHEN_CLIENT_SPECIFIC_LOGIN_AND_LOGIN_TO_CLIENT_FAILED
        ),
        switchMap((_) =>
          combineLatest([
            this.centralAuthFacade.isClientSpecificLogin$.pipe(take(1)),
  ...
  authorize$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action: { type: string; accessToken: string }) =>
        combineLatest([
      ...
      switchMap(([accessToken, redirectUri]) =>
        this.authService
          .retrieveAuthorizationCode(redirectUri, accessToken)
          .pipe(
            map((response) =>
  ...
  authorizeSuccess$ = createEffect(() =>
    this.actions$.pipe(
      ...
      filter(
        ([redirectionUri, authorizationCode]) =>
          !!redirectionUri && !!authorizationCode
      ),
      tap(
        ([
          ...
          const url = UtilsClass.generateClientUrl(
            redirectionUri,
            authorizationCode
          );
          openClientInNewTab
...
export class UtilsClass {
  static generateClientUrl(
    ...
    if (
      decodedRedirectUri.includes('v06') &&
      decodedRedirectUri.includes('ReturnUrl')
    ) {
      const baseUrl = decodedRedirectUri.substring(
        0,
        decodedRedirectUri.indexOf('ReturnUrl') + 10
      );
      const returnUrl = decodedRedirectUri.substring(
        decodedRedirectUri.indexOf('ReturnUrl') + 10
      );
      decodedRedirectUri = `${baseUrl}${encodeURIComponent(returnUrl)}`;
    }
    const url = this.addQueryParamWithProperSymbol(
      decodedRedirectUri,
      OAUTH_AUTH_CODE_QUERY_PARAM,
      authorizationCode
    );
