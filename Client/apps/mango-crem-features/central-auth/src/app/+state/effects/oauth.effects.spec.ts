...
import { Environment, OAUTH_AUTH_CODE_QUERY_PARAM } from '@mango/data-models/lib-data-models';
...
describe('OAUth Effects', () => {
  ...
  describe('when AUTHORIZE_SUCCESS action is dispatched', () => {
    ...
    beforeEach(() => {
      actions$ = of(OAuthActions.authorizeSuccess({authorizationCode: 'mock_authorization_code'}))
      state.overrideSelector(AppSelectors.redirectionUri, 'https://url.com')
      state.overrideSelector(AppSelectors.authorizationCode, 'mock_authorization_code')
...
describe('UtilsClass', () => {
  ...
  describe('#generateClientUrl', () => {
    ...
    it('should add authorization code to the client url', done => {
      const mockRedirectionUrl = encodeURIComponent('http://localhost:4201/auth/validate?redirect_uri=/crem/projects/project-dashboard/')
      const mockAuthorizationCode = 'authorization_code_mock'
      const clientUrl = UtilsClass.generateClientUrl(mockRedirectionUrl, mockAuthorizationCode)
      expect(clientUrl).toStrictEqual(`http://localhost:4201/auth/validate?redirect_uri=/crem/projects/project-dashboard/&${OAUTH_AUTH_CODE_QUERY_PARAM}=${mockAuthorizationCode}`)
      done()
    })

    it('should encode ReturnUrl when redirectUrl is a v06 link', done => {
      const mockRedirectionUrl = encodeURIComponent('http://blank.dev.corp.virtualpremise.com/v06/Login.aspx?ReturnUrl=/v06/Admin/AdminHome2.aspx')
      const encodedV06ReturnUrl = '%2Fv06%2FAdmin%2FAdminHome2.aspx'
      const mockAuthorizationCode = 'authorization_code_mock'
      const clientUrl = UtilsClass.generateClientUrl(mockRedirectionUrl, mockAuthorizationCode)
      expect(clientUrl).toStrictEqual(`http://blank.dev.corp.virtualpremise.com/v06/Login.aspx?ReturnUrl=${encodedV06ReturnUrl}&${OAUTH_AUTH_CODE_QUERY_PARAM}=${mockAuthorizationCode}`)
      done()
    })
    ...
    describe('#accountForQueryParamAsFirstOrSecondParam', () => {
      ...
      it('should add query param with ? symbol when it is the first query param in the url', done => {
        ...
        const key = 'key1'
        const value = 'value1'
        const adjustedUri = UtilsClass.addQueryParamWithProperSymbol(uri, key, value)
      ...
      it('should add query param with & symbol when it is not the first query param in the url', done => {
        ...
        const value = 'value1'
        const adjustedUri = UtilsClass.addQueryParamWithProperSymbol(uri, key, value)
        expect(adjustedUri).toStrictEqual('https://url.com?param1=v&key1=value1')
        done()
      })
