...
import {
  ...
  ContactRecord,
  MangoErrorTypes,
  V06_LOGIN_ERROR_MESSAGE,
  USER_LOGGED_OUT_ERROR_MESSAGE,
...
export class AppEffects {
      ...
      switchMap((_) =>
        this.authService.getCurrentUser().pipe(
          ...
          map((user) => {
            ...
            return AppActions.setLoadCurrentUserComplete({
              loadCurrentUserComplete: true,
            });
          }),
          catchError((_) => {
            return of(
  ...
  setupLogoutWhenTimedOut$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((_) => [
        dayjs(this.storageService.getData(DBkeys.IDLE_TIMEOUT)),
        dayjs(new Date()),
      ]),
      filter(
        ([idleTimeoutDate, currentDate]) =>
          currentDate.diff(idleTimeoutDate) > 0
      ),
      map((_) => {
      ...
      this.actions$.pipe(
        ...
        tap((_) =>
          window.addEventListener(
            'storage',
            this.logoutStorageEventListener.bind(this)
          )
        )
      ),
    { dispatch: false }
  ...
  populateSelectedClientAndContactRecords$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action: { type: string; clientKey: string }) =>
        combineLatest([
          this.centralAuthFacade.user$.pipe(first((user) => !!user)),
          this.centralAuthFacade.userClients$.pipe(
            first((clients) => !!clients)
          ),
          of(action.clientKey),
        ])
      ),
      filter(
        ([user, clients, clientKey]) => !!user && !!clients && !!clientKey
      ),
      map(([user, clients, clientKey]) =>
        clients.find(
          (client) => client.clientKey.toLowerCase() === clientKey.toLowerCase()
        )
      ),
      filter((selectedClient) => !!selectedClient),
      switchMap((selectedClient) =>
        of(
          AppActions.setSelectedClient({ client: selectedClient }),
          AppActions.getContactRecords({ clientKey: selectedClient.clientKey })
        )
      )
    )
  );
  ...
  populateSelectedContactRecord$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action: { type: string; contactId: number }) =>
        combineLatest([
      ...
      filter(
        ([user, contactRecords, selectedContactId]) =>
          !!user && !!contactRecords && !!selectedContactId
      ),
      map(([user, contactRecords, selectedContactId]) =>
        contactRecords.find(
          (contacRecord) => contacRecord.contactID === selectedContactId
        )
      ),
      filter((selectedContactRecord) => !!selectedContactRecord),
      map((selectedContactRecord) =>
        AppActions.setContactRecord({ contactRecord: selectedContactRecord })
      )
    )
  );
  ...
  startAuthorizationWhenFullySelected$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([client, contactRecord]) => {
        if (
          client.isSSOEnabled &&
          (client.forceSSO || contactRecord.requireSSO)
        ) {
