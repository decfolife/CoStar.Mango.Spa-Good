...
export class HttpEffects {
  ...
  getUserClients$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((_) =>
        combineLatest([this.centralAuthFacade.user$.pipe(take(1))])
      ),
      filter(([user]) => !!user),
      switchMap(([user]) => this.authService.getClientSitesByUser(user.email)),
      map((response) =>
        AppActions.getUserClientsSuccess({ clientSites: response })
      ),
      finalize(() => {
  ...
  getContactRecords$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([clientKey, user]) =>
        this.authService.getContactRecords(user.email, clientKey)
      ),
      map((response) =>
        AppActions.getContactRecordsSuccess({
          contactRecords: response.contactRecords.map(contactRecordMapper),
        })
      ),
      finalize(() => {
        this.centralAuthFacade.setLoading(false);
  ...
  getContactRecordsSuccess$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(
        (action: { type: string; contactRecords: ContactRecord[] }) =>
          action.contactRecords
      ),
      filter(
        (contactRecords) => !!contactRecords && contactRecords.length <= 1
      ),
      switchMap((contactRecord) =>
        combineLatest([
          of(contactRecord),
          this.centralAuthFacade.isSwitchContactRecord$.pipe(take(1)),
        ])
      ),
      switchMap(([contactRecord, isSwitchContactRecord]) => {
        ...
        return contactRecord.length === 0
          ? of(
              AppActions.setSelectedContactID({ contactId: 0 }),
              AppActions.setContactRecord({ contactRecord: { contactID: 0 } })
            )
          : of(
              AppActions.setSelectedContactID({
                contactId: contactRecord[0].contactID,
              })
            );
      })
    )
  ...
  getClientSSSOSettings$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action: { type: string; clientKey: string }) =>
        this.settingsService.getClientSsoSettings(action.clientKey)
      ),
      map((response) =>
        AppActions.getClientSSOSettingsSuccess({ ssoSettings: response })
      )
    )
  );
}
...
// To be refactored
function contactRecordMapper(
  contactRecordHttpObject: ContactRecordHTTPObject
): ContactRecord {
  const {
    contactID,
