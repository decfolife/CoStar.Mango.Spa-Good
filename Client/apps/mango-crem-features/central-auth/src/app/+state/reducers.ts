...
export const appAdapter: EntityAdapter<CentralAuthEntity> =
  createEntityAdapter<CentralAuthEntity>();
...
const appReducer = createReducer(
  ...
  on(AppActions.loginError, (state) => ({
    ...state,
    error: true,
    loading: false,
  })),
  ...
  on(
    AppActions.setLoadCurrentUserComplete,
    (state, { loadCurrentUserComplete }) => ({
      ...state,
      loadCurrentUserComplete,
    })
  ),
  on(AppActions.setSelectedClientKey, (state, { clientKey }) => ({
    ...state,
    selectedClientKey: clientKey,
  })),
  on(AppActions.setSelectedClient, (state, { client }) => ({
    ...state,
    selectedClient: client,
  })),
  on(AppActions.setSelectedInstance, (state, { instance }) => ({
    ...state,
    selectedInstance: instance,
  })),
  on(AppActions.setSelectedContactID, (state, { contactId }) => ({
    ...state,
    selectedContactId: contactId,
  })),
  on(AppActions.setDefaultContactRecord, (state, { defaultContact }) => ({
    ...state,
    selectedDefaultContactRecord: defaultContact,
  })),
  on(AppActions.setContactRecord, (state, { contactRecord }) => ({
    ...state,
    selectedContactRecord: contactRecord,
  })),
  on(AppActions.getUserClients, (state, _) => ({
    ...state,
    error: false,
    loading: true,
  })),
  on(AppActions.getUserClientsSuccess, (state, { clientSites }) => ({
    ...state,
    error: false,
    loading: false,
    userClients: clientSites.userSites,
    userRecentClients: clientSites.recentUserSites,
  })),
  on(AppActions.getClientSSOSettingsSuccess, (state, { ssoSettings }) => ({
    ...state,
    error: false,
    ssoSettings,
  })),
  on(AppActions.setRedirectionUri, (state, { redirectionUri }) => ({
    ...state,
    redirectionUri,
  })),
  on(AppActions.setClientSpecificLogin, (state, { isClientSpecific }) => ({
    ...state,
    isClientSpecificLogin: isClientSpecific,
  })),
  on(AppActions.setOpenClientInNewTab, (state, { openClientInNewTab }) => ({
    ...state,
    openClientInNewTab,
  })),
  on(
    AppActions.setIsSwitchContactRecord,
    (state, { isSwitchContactRecord }) => ({ ...state, isSwitchContactRecord })
  ),
  on(OAuthActions.authorize, (state) => ({ ...state, loading: true })),
  on(OAuthActions.authorizeSuccess, (state, { authorizationCode }) => ({
    ...state,
    error: false,
    loading: false,
    authorizationCode,
  })),
  on(OAuthActions.authorizeError, (state) => ({
    ...state,
    error: true,
    loading: false,
  })),
  on(AppActions.getContactRecords, (state, _) => ({
    ...state,
    error: false,
    loading: true,
  })),
  on(AppActions.getContactRecordsSuccess, (state, { contactRecords }) => ({
    ...state,
    error: false,
    loading: false,
    userContactRecords: contactRecords,
  })),
