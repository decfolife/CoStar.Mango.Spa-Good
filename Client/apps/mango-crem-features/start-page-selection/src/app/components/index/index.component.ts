...
export class IndexComponent implements OnInit {
  ...
  // Dropdown list items
  getStartPagesList() {
    this.starPageSvc.getDefaultStartPagesList().subscribe(
      (response: any) => {
        if (response.success) {
  ...
  getModulesList(modulesData: any) {
    const hasModules = modulesData.some(
      (obj) => obj.moduleID == 1 || obj.moduleID == 2
    );
    if (hasModules) {
      this.modulesList = modulesData.filter(
        (data) =>
  ...
  startPageSelected(selectedItem) {
    let selectedPage: UserSelectedPageData = {
      defaultStartPageModuleId: selectedItem.moduleID,
      defaultStartPageUrl: selectedItem.pagePath,
    };
    this.starPageSvc.saveDefaultStartPage(selectedPage).subscribe(
      (response: any) => {
        if (response.success) {
          sessionStorage.setItem(
            `FramelessCurrentModuleId`,
      ...
      (error: any) =>
        console.log('Error occurred saving user selected page: ', error),
      () => {}
    );
  }
}
