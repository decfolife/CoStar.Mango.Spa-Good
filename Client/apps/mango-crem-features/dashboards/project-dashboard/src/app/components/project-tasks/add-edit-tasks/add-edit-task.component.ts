...
import { AvailableRole, ContactRecord, MemberInfo, ProjectTaskDropdownInfo, ProjectTaskInfo, CreateProjectTask, Predecessor, ProjectTaskSettings } from '@mango/data-models/lib-data-models';
import { DatePickerComponent, InputComponent } from '@mango/ui-shared/lib-ui-elements';
...
export class AddEditTaskComponent implements OnInit {
  ...
  @ViewChild("TargetStartDatePicker") targetStartDatePicker: DatePickerComponent;
  ...
  populateTaskInfo() {
    ...
    this.taskRequiredApprovals = this.projectTaskDetails.taskInfo.requiredApprovers;
    ...
    this.selectedTaskName = this.projectTaskDetails.taskInfo.projectMilestoneName;
    this.targetStartDate = this.projectTaskDetails.taskInfo.projectMilestoneStartDate;
    this.targetEndDate = this.projectTaskDetails.taskInfo.projectMilestoneEndDate;
    this.taskDaysToStart = this.projectTaskDetails.taskInfo.projectMilestoneDaysToStart;
    this.taskDuration = this.projectTaskDetails.taskInfo.projectMilestoneDaysToComplete;
    ...
    this.taskWorkflowStatusID = this.projectTaskDetails.taskInfo.workflowStatusID; 
    this.taskWorkflowName = this.projectTaskDetails.taskInfo.workflowName;
    this.taskPercentCompletion =  this.projectTaskDetails.taskInfo.percentComplete;
    this.taskRequired = this.projectTaskDetails.taskInfo.projectMilestoneRequired;
    ...
    this.taskApprovalExempt = this.projectTaskDetails.taskInfo.approvalExempt ?? false;
    this.requiredApprovers = this.projectTaskDetails.taskInfo.requiredApprovers ?? 0;
    this.taskDescription = this.projectTaskDetails.taskInfo.projectMilestoneDescription 
                          ? this.projectTaskDetails.taskInfo.projectMilestoneDescription.replace(/<br>/g, '\n') 
                          : this.projectTaskDetails.taskInfo.projectMilestoneDescription;
    this.availablePredecessors = this.projectTaskDetails.taskInfo.potentialPredecessors;
    this.taskSelectedPredecessors = this.projectTaskDetails.taskInfo.existingPredecessors.map(ea => ea.projectMilestoneID);
  }
  ...
  getPredessors() {
    ...
    this.projectCurrentTasks.forEach((task) => {
      if(this.taskIndexOrder >= task.indexOrder ) {
          ...
          tempPredecessor.projectMilestoneStepFullAndName = task.taskFullStep + " - " + task.taskName;
  ...
  validateAndSaveTask(saveOrApply) {
    ...
    this.taskTargetStartDateFieldInvalid = !this.targetStartDatePicker.validate();
    this.taskTargetEndDateFieldInvalid = !this.targetEndDatePicker.validate();

    if(this.taskNameFieldInvalid || this.taskTargetStartDateFieldInvalid || this.taskTargetEndDateFieldInvalid) {
  ...
  saveTask(saveOrApply) {
    ...
    else if(this.addSubTask)
      parentIdToSave = this.data.taskId;
    ...
    this.createOrUpdateProjectTask.step = this.addSubTask ? 1 : this.taskStep + 1;
    this.createOrUpdateProjectTask.daysToStart = this.taskDaysToStart;
    this.createOrUpdateProjectTask.calc = this.projectTaskSettings.calculateDates ? 1 : 0;
    this.createOrUpdateProjectTask.autoCalc = this.projectTaskSettings.autoCalculate ? 1 : 0;
    this.createOrUpdateProjectTask.isChild = this.addSubTask ? 1 : 0;
    this.createOrUpdateProjectTask.useEmailApprovals = this.clientEmailPreference;
    this.createOrUpdateProjectTask.name = this.taskName;
    this.createOrUpdateProjectTask.description = this.taskDescription?.trim().length > 0 ? this.taskDescription.trim() : null;
    ...
    this.createOrUpdateProjectTask.requiredApprovers = this.taskRequiredApprovals;

    this.subs.push(this.dashboardService.createOrUpdateTask(this.createOrUpdateProjectTask).subscribe(
      (res:any) => {
        if(res && res.success) {
          ...
          this.subs.push(this.dialogService.alert('Create or Update Project Task', 'Error Creating or Updating Project Task. Please review and try again later', 'OK').subscribe());
        }
      },
      (error: any) => console.log("Error occurred Creating or Updating Project Task ", error),
      () => {}
    ));
  }
  ...
  changeToEditPopup(taskId: number, projectCurrentTasks: ProjectTaskDropdownInfo[]) {
  ...
  private getClientEmailPreference() {
    this.subs.push(this.dashboardService.getClientPreference('ClientProjectEmailOn').subscribe(
      (res:any) => {
        if (res.success) {
  ...
  private getWorkflowStatuses() {
    this.subs.push(this.dashboardService.getWorkflowStatuses().subscribe(
      (res:any) => {
        if (res.success) {
          this.taskWorkFlows = res.data;  
        }
      },
      (error: any) => console.log("Error occurred getting the Workflow Statuses", error),
      () => {}
    ));
  }
