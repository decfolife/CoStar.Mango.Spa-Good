...
export class TaskAssigneesComponent {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if(!!changes.taskAssignees && !!changes.taskAssignees.currentValue) {
      ...
      this.approvedCount = this.taskAssignees.filter(ta => !!ta.approvalDate).length;
      this.rejectedCount = this.taskAssignees.filter(ta => !!ta.rejectDate).length;
      this.pendingCount = this.totalCount - (this.approvedCount + this.rejectedCount);
    }
  }
}
