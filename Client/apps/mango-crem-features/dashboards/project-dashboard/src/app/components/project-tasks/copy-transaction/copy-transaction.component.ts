...
export class CopyTransactionComponent implements OnInit, OnChanges {
  ...
  @Output() copyCompletedEvent: EventEmitter<{
    success: boolean;
    newProjectId?: number;
  }> = new EventEmitter();
  ...
  OnCheckboxChangeEvent(e, option) {
    ...
    if (
      option == 'selectApprovers' ||
      option == 'selectTaskNotes' ||
      option == 'selectTaskFiles'
    ) {
      if (e.value == true) {
  ...
  getProjectTemplates() {
    this.subs.push(
      this.dashboardService.getProjectTemplateList().subscribe((res: any) => {
        if (res && res.success) {
          if (res.data.length) {
            this.projectTemplatesList = this.projectTemplatesList.concat(
              res.data
            );
          }
        } else {
  ...
  getProjectTemplateTasks(templateId: number) {
    this.subs.push(
      this.dashboardService
        .getProjectTemplateTaskList(templateId)
        .subscribe((res: any) => {
          if (res && res.success) {
            this.projectTemplateTaskList = res.data;
          } else {
            this.dialogService.alert(
              'Get Project Template Tasks Error',
              'There was an issue with getting the Project Template Tasks Info. Please contact the system administrator.',
              'OK'
            );
          }
        })
        ...
        .subscribe((res: any) => {
          if (res && res.success) {
            ...
            this.copyCompletedEvent.emit({
              success: true,
              newProjectId: res.data,
            });
            this.toastService.show(
              'Copy Transaction was successful.',
              'Success',
              ToastState.SUCCESS
            );
          } else {
            this.copyCompletedEvent.emit({ success: false });
            this.dialogService.alert(
              'Post Copy Transaction Error',
              'There was an issue with Copy Transaction operation. Please contact the system administrator.',
              'OK'
            );
          }
        })
