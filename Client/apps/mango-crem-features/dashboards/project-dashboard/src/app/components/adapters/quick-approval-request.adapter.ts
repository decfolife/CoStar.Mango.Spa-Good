...
export const adaptResponseToUI = (approvals): QuickApprovalUI[] => {
    const mappedResponse: QuickApprovalUI[] = approvals.map(approval => {
        return {
            ...approval,
            actualStartDate: approval.actualStartDate ? new Date(approval.actualStartDate): null,
...
export const buildQuickApprovalRequest = (selectedTasksToApprove: any[]): QuickApprovalRequest => {
  const mappedRequest: QuickApprovalRequest = {
    ...
    approveRejectTasksRequestList: selectedTasksToApprove.map((task) => {
      return {
        ...
        actualStartDate: task.actualStartDate ? task.actualStartDate.toISOString() : null,
        newHours: task.newHours ? task.newHours : 0,
        userDate: task.date ? task.date.toISOString() : null,
        isApproval: true,
        isProxyApproval: task.isProxyApproval,
        notes: task.note,
      };
    }),
  };
