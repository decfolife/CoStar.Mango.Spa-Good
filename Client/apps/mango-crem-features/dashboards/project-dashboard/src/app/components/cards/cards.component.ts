...
export class CardsComponent implements OnInit {
  ...
  setFocus() {
    let cardIndex = this.cardsList
      .toArray()
      .findIndex(
        (item) =>
          item.nativeElement.cdkDropListData.card.elementId ==
          this.activeCardElementId
      );
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    this.cards.forEach((card, index) => {
      userSettingsData[index] = this.cardsService.createUserSettingRec(
        card,
        index
      );
    });
    ...
    return this.dashboardService.postUserSettings(userSettingsData).subscribe(
      (returnData: any) =>
        console.log('Returned results after card dragNDrop: ', returnData),
      (error: any) =>
