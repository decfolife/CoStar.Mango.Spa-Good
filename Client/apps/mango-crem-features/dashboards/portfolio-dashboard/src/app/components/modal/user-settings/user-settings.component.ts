...
export class UserSettingsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.filters) { this.filtersDisplay = this.sorted(this.filters.map(this.extract)); }
    if (this.metrics) { this.metricsDisplay = this.sorted(this.metrics.map(this.extract)); }
    if (this.cards) { this.cardsDisplay = this.sorted(this.cards.map(this.extract)); }
  ...
  toggleChangeEvent(event, currElement: any, elements: any) {
    ...
    userSettingsData.push(this.portfolioDataService.createUserSettingRec(element, element.elementOrder));
    ...
    if (element.isActive && (element.dataSource === null || element.dataSource === undefined)) {
      if (currElement.elementTypeName.endsWith("_filter"))
        ...
        this.subs.push(this.portfolioDataService.getDataForFilterDetail(element).subscribe());
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    this.cards.forEach((card, index) => {
      this.cardUserSettingsData[index] = this.portfolioDataService.createUserSettingRec(card, index);
    })
  ...
  populateUOMAndCurrency() {
    ...
    this.subs.push(this.portfolioDashboardService.getGetUOMAndCurrency().subscribe(
      (res: any) => {
        if (res.success) {
      ...
      (error: any) => console.log("Error occurred getting Currency and Unit of Measure Data: ", error),
      () => { }
    ));
  }

  getStoragePreferences() {
    let currency = sessionStorage.getItem(`PortfolioDashboardCurrency_${this.userId}`);
    let unitOfMeasure = sessionStorage.getItem(`PortfolioDashboardUOM_${this.userId}`);
  ...
  applySettings() {
    if (this.cardOrderChanged) {
      return this.subs.push(this.portfolioDashboardService.postUserSettings(this.cardUserSettingsData).subscribe(
        (returnData: any) => (console.log("Returned results after card dragNDrop: ", returnData)),
        (error: any) => console.log("Error occurred updating userSettings: ", error)
      ));
    }

    sessionStorage.setItem(`PortfolioDashboardCurrency_${this.userId}`, this.selectedCurrency);
    sessionStorage.setItem(`PortfolioDashboardUOM_${this.userId}`, this.selectedUnitOfMeasure);
    this.portfolioDataService.exchangeRateId = Number(this.selectedCurrency);
    this.portfolioDataService.unitOfMeasureId = Number(this.selectedUnitOfMeasure);
  }
