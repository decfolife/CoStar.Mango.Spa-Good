...
export class GroupMaintenanceComponent implements OnInit {
  ...
  exportTreeList() {
    ...
    const groupMaintenanceSheet = workbook.addWorksheet('Security Groups', {
    ...
    groupMaintenanceSheet.getRow(2).getCell(2).font = {
      bold: true,
      size: 16,
      underline: 'double',
    };
  ...
  private setColumnHeaderStyle(
    ...
    groupMaintenanceSheet.getRow(row).getCell(column).font = {
      color: { argb: '00558E' },
    };
    groupMaintenanceSheet.getRow(row).getCell(column).fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: 'd2d2d2' },
      bgColor: { argb: 'd2d2d2' },
        ...
        item.childItems.forEach((childItem1) => {
          ...
          if (
            childItem1.data.companyID.toString().length +
              childItem1.data.companyName.length -
              13 >
            maxWidth
          ) {
            maxWidth =
            ...
            childItem1.childItems.forEach((childItem2) => {
              ...
              if (
                childItem2.data.companyID.toString().length +
                  childItem2.data.companyName.length -
                  9 >
                maxWidth
              ) {
                maxWidth =
                ...
                childItem2.childItems.forEach((childItem3) => {
                  ...
                  if (
                    childItem3.data.companyID.toString().length +
                      childItem3.data.companyName.length -
                      5 >
                    maxWidth
                  ) {
                    maxWidth =
                    ...
                    childItem3.childItems.forEach((childItem4) => {
                      ...
                      if (
                        childItem4.data.companyID.toString().length +
                          childItem4.data.companyName.length -
                          1 >
                        maxWidth
                      ) {
                        maxWidth =
                        ...
                        childItem4.childItems.forEach((childItem5) => {
                          ...
                          if (
                            childItem5.data.companyID.toString().length +
                              childItem5.data.companyName.length +
                              3 >
                            maxWidth
                          ) {
                            maxWidth =
  ...
  private addCellData(groupMaintenanceSheet, nameCell, row, column, item) {
    groupMaintenanceSheet.getRow(row).getCell(column).value =
      '(' + item.data.companyID + ') ' + item.data.companyName;
  ...
  public getUserHaveAdminRights() {
    this.service.getHasAdminLinkRights().subscribe((result) => {
  ...
  public setSearchField() {
    this.groupMaintenanceData.forEach((data) => {
      ...
      if (data.parentGroupID) {
        searchField = this.getParentDisplayName(
          searchField,
          data.parentGroupID
        );
      }
  ...
  public getParentDisplayName(searchName, id: number) {
    ...
    if (parentItem.parentGroupID) {
      searchName = this.getParentDisplayName(
        searchName,
        parentItem.parentGroupID
      );
    }
    return searchName;
  }
  ...
  public redirectToAction(companyId, portfolioId, action) {
    ...
    switch (action) {
      case 'addSubGroup': {
        route = `${
          this.basepageUrl
        }editGroup/11/0${basewebPage}/${companyId.toString()}/11`;
        break;
      }
      case 'addGroup': {
      ...
      case 'edit': {
        route =
          companyId === portfolioId
            ? `${this.basepageUrl}editPortfolio/${companyId.toString()}/${
                companyId.toString() + basewebPage
              }`
            : `${this.basepageUrl}editGroup/11/${
                companyId.toString() + basewebPage
              }`;
        break;
      }
      default: {
        route = `${this.basepageUrl + action}/11/${
          companyId.toString() + basewebPage
        }`;
        break;
      }
