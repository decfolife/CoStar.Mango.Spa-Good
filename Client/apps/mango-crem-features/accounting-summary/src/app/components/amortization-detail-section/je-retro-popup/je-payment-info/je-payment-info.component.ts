...
export class JePaymentInfoComponent {
  ...
  chargeAmountFormatter = (value: any) =>
    this.formattingService.localFormat(
      +value,
      this.jePaymentPopupData[0].chargeCurrencyDecimalPrecision
    );
  scheduleAmountFormatter = (value: any) =>
    this.formattingService.localFormat(
      +value,
      this.jePaymentPopupData[0].scheduleCurrencyDecimalPrecision
    );

  constructor(
    public accountingSummaryService: AccountingSummaryService,
    public formattingService: FormattingService,
    public jePaymentGridColumnsService: AmortizationGridColumnsService
  ) {}
  ...
  jePaymentInfoGridSetup() {
    ...
    this.jePaymentGridColumns.forEach((column) => {
      ...
      if (
        ...
        column.name === 'DirectCosts' ||
        column.name === 'TerminationFees'
      ) {
        column.format = (value) =>
          this.formattingService.localFormat(
            +value,
  ...
  getFormattedDisplay(target: 'summaryRow' | 'columnCaption'): string {
    ...
    const getCurrency =
      delimiter.length > 1 ? delimiter[1]?.trim() : currency.trim();
