...
export class RetrospectiveAdjustmentInfoComponent {
  @ViewChild('RetrospectiveAdjustment')
  retrospectiveAdjustment: DxDataGridComponent;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      !!changes.gridColumnsForRetroPopup &&
      changes.gridColumnsForRetroPopup.currentValue !== undefined
    ) {
  ...
  onExporting(event) {
    ...
    this.accountingSummaryService.exportToExcel(
      this.retrospectiveAdjustment.instance,
      fileName,
      'Sheet'
    );
  }
