...
export class EndpointService {
  protected httpOptions: IHttpOptions = {
  ...
  protected callHttpGet(url: string, functionName: string, httpOptionsParams?: HttpParams | {[param: string]: any}): Observable<any> {
  ...
  protected handleApiResponseError(operation = 'operation not provided') {
    return (error: any): Observable<any> => {
      ...
      if (environment.isRestful && error) {
        if (operation === 'Archive' && error && error.error) {
          var apiResponse = error.error;
          return of({
            success: apiResponse.success,
            data: apiResponse.clientErrorMessage,
            clientErrorMessage: error.status
          });
        }
        return of({
          success: false,
  ...
  protected toApiResponse(value: any): ApiResponse {
    ...
    let res = value.d.hasOwnProperty('Result') && value.d.Result !== "" ? JSON.parse(value.d.Result) : value.d;
