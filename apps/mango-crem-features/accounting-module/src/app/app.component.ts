...
export class AppComponent {
  ...
  constructor(
    ...
    this.page = this.sanitizer.sanitize(SecurityContext.HTML, this.elementRef.nativeElement.getAttribute('page'));
    this.api = this.sanitizer.sanitize(SecurityContext.HTML, this.elementRef.nativeElement.getAttribute('api'));
    ...
    return this.leftNavService.getModuleNavigationLinksClient(9).pipe(
      map(res => {
        for (const link of res.data) {
          if (link.isActive === false && (link.name === 'Dashboard' || link.name === 'Accounting Events')) {
            if (route.routeConfig.path === link.name.toLowerCase().replace(' ', '')) {
