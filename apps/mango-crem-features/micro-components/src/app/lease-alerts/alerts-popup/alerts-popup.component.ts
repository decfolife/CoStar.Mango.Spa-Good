...
export class AlertsPopupComponent implements OnInit {
  ...
  getButtonClass(): string {
    let classString = this.severityLevel === 'critical'
      ? 'danger'
  ...
  getCachedDateString(): string {
    if (this.cachedDate) {
      return this.alertsService.isEuroDateFormat ?
        'Last updated ' + format(this.cachedDate, 'dd.MM.yyyy HH:mm:ss') : 'Last updated ' + format(this.cachedDate, 'M/d/yyyy h:mm:ss aa');
    }
    return '';
  }

  private getAlertCountAndSeverityLevel(): void {
    this.alertsService.getUndismissedLeaseAlertsStats(this.leaseAbstractID)
      .subscribe((response: ApiResponse) => {
        ...
        if(res.cachedDate)    
          this.cachedDate = new Date(res.cachedDate);
      });
  }
}
