...
export class PaymentDetailsPopupComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.chargeDetails = this.paymentDetails.chargeDetails ? this.paymentDetails.chargeDetails : <ChargeDetails>{};

    if((this.paymentDetails.portfolioSegments && this.paymentDetails.portfolioSegments.length) && 
        (this.paymentDetails.chargeAllocations && this.paymentDetails.chargeAllocations.length)) {
