...
export class CardsService {
  ...
  generateCardDetails(cards: any, cardFilters: any, selectedFilters: string): CardDetails[] {
  ...
  getDataForFilterDetail(filterDetail: FilterDetail): Observable<FilterDetail>{
    ...
    return this.dashboardService.getFilterDataByElementType(1, filterDetail.id
    )
    .pipe(
      map((filterData: any) => {
  ...
  generateMetricDetails(dashboardId, metrics, data): any {
    ...
    metrics.forEach(metric => {
      let metricDataIndex = data.findIndex((m) => m.elementTypeName === metric.elementType.elementTypeName);
  ...
  getDataForCardDetail(cardDetail: CardDetails, keyDate: string, selectedFilters: string): Observable<CardDetails> {
