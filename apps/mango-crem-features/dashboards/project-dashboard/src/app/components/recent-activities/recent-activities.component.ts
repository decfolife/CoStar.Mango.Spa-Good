...
export class RecentActivitiesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dashboardService.getRecentActivities(durationDays).subscribe(
      ...
      (error: any) => console.log("Error occurred getting Recent Activities: ", error),
      () => {}
    );
  ...
  getObjectType(){
    this.dashboardService.getObjectTypeNames(this.objectTypeId).subscribe(
      (res:any) => {
  ...
  isActivityNoteAdded(cell: any){
    return cell.data.activity && cell.data.activity.toLowerCase() !== 'file upload';
  }
  ...
  downloadfile(fileInformation: any): boolean {
    this.dashboardService.getActivityFeedFile(fileInformation.data.description).subscribe((response: any) => {
