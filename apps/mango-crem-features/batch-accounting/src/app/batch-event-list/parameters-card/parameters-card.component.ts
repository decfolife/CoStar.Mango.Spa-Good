...
export class ParametersCardComponent implements OnInit {
  ...
  private filterNextWorkflowStatusDropdown(): void {
    const statuses = this.parametersData.workflowStatuses
      ...
      ?.filter(ws => (ws as any).userHasRights || (ws as any).allUsersHaveRights)
    ...
    this.nextWorkflowStatuses = index < this.parametersData.workflowStatuses.length - 1
  ...
  private filterMeasureEventDropdown(): void {
    ...
    const classIds = this.scheduleObject?.ClassificationTypes
      ?.map(x => x.classificationID);

    this.measureEvents = this.parametersData.measureEvents.map((measureEvent) => {
    ...
    classIds.forEach(id => {
      if ([ 0, 1, 5 ].includes(id)) { // Operating840, Capital840, OperatingLessor
      ...
      if ([ 2, 3 ].includes(id)) { // Finance842, Operating842
        this.measureEvents.map(x => {
          if ([ RemeasureTypes[ 'Rent Review (IFRS)' ] ].includes(x.remeasureTypeId)) {
            x.visible = false;
          }
        });
      }
    ...
    if (this.parametersData.cardMeasureEvent) {
      ...
      this.parametersData.cardMeasureEvent = this.measureEvents
        .find(x => x.remeasureTypeId === eventId);
    }
  }
}
